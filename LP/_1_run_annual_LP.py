...
# only one SMR 
# don't sample random error, simply use expected cost

#could change from "fast transition" and slow transition 
...
# Helper to ensure SMR is initialized when it appears in 2030
...
for y in years:
    ...
    if y >= 2030:
        # Add SMR starting in 2030
        ensure_smr_initialized()
        techs_y.append(SMR_TECH)
        # In 2029 file, we want previous_gen seed for SMR = 0.058; we will write it when y==2029 below
    ...
    with open(f"model_params/param_{y}.dat", "w") as f:
        ...
        # Build previous values with safe fallbacks for first appearance (e.g., SMR in 2030)
        ...
        for t in techs_y:
            # Seed SMR when it first appears (2030)
...
with open('000_results_lp_trajectories.json', 'w') as f:
